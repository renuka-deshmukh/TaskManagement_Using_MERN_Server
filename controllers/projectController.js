...
async function getAllProjects(req, res) {
    ...
    try {
        ...
        const projects = await Project.find().populate("addedBy", "name email");
...
async function createProject(req, res) {
    try {
        ...
        // Check if user info is attached from middleware
        if (!req.user || !req.user._id) {
            return res.status(401).json({ success: false, message: "Not authorized" });
        }

        // Create new project with addedBy automatically set
        const newProject = await Project.create({
            ...
            endDate,
            status,
            addedBy: req.user._id, // ✅ Set automatically from token
        });
